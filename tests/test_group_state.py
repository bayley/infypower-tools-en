...
def test_module_alarms_decodes_status0_bits():
    ...
    s = m.alarms
    assert "Communication loss alarm" in s
    assert "Airflow blocked" in s
    assert "Module discharge abnormal" in s
    assert "Input or bus abnormal" in s
...
def test_module_alarms_decodes_status1_bits():
    ...
    assert "Overtemperature alarm" in m.alarms
    assert "Module fault alarm" in m.alarms


def test_module_alarms_decodes_status2_bits():
    from group_state import ModuleState
    m = ModuleState(addr=0x00, status2=(1 << 7) | (1 << 6))
    assert "Module PFC side is off" in m.alarms
    assert "Input overvoltage alarm" in m.alarms
